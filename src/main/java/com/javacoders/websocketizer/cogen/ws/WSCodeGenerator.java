 ...
 * service represented by this {@link RestServiceBlueprint}. The service blueprint
...
public class WSCodeGenerator implements CodeGenerator {
   ...
   */
  @Override
  public void generate(ServiceBlueprint serviceBlueprint, File dir) throws IOException {
    try {
    ...
    JavaFile file = JavaFile.
        builder(sb.getPackageName() + ".autogenerated", constructWebSocketServerClass(sb, onOpen, onMessage, onClose))
        .build();
    
    file.writeTo(new File(sb.getSourceDir()));
    System.out.println(sb.getAutogeneratedPath());
    createFrameworkFile(WS_REQUEST, "WSRequest.java", sb.getPackageName(), sb.getAutogeneratedPath());
    createFrameworkFile(UTIL, "Util.java", sb.getPackageName(), sb.getAutogeneratedPath());
    correct++;
  ...
  private static TypeSpec constructWebSocketServerClass(RestServiceBlueprint sb, MethodSpec onOpen, MethodSpec onMessage, MethodSpec onClose) {
    AnnotationSpec serverEndpointAnnotation = AnnotationSpec.builder(ServerEndpoint.class)
        .addMember("value", "$S", sb.getEndpoint())
        .build();
  ...
  private static MethodSpec constructOnMessageMethod(RestServiceBlueprint sb) {
     ...
     Builder methodBuilder = MethodSpec
        ...
        .addStatement("$T<Object> params = " + sb.getPackageName() + ".autogenerated.Util.parseMessage(message, " + getRequestBodyClass(sb) + ".class)", List.class);
     ...
     methodBuilder
        .addStatement("session.getBasicRemote().sendText("+ sb.getPackageName() +".autogenerated.Util.getJson(" + constructMethodInvocation(sb) + "))")
        .addParameter(Session.class, "session");
